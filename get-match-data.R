...
get_match_data <- function(player_ids, num_matches) {
  message("Retrieving match data")
  ...
  if (file.exists("data/match_cache.csv")) {
    cached_data <- read.csv(
      file = "data/match_cache.csv",
      header = TRUE,
      colClasses = "numeric"
    )
  } else {
  ...
  match_data <- all_data %>% 
    inner_join(match_list, by = c("player_id", "match_id"))
